...
final class ActionComposer private (act: GameState => GameState) {
  ...
  def >>(next: GameAction): ActionComposer = new ActionComposer(gameState =>
    next(this.act(gameState))
  )

  def >>>(next: GameState => Vector[GameAction]): ActionComposer =
    new ActionComposer((gameState: GameState) => {
