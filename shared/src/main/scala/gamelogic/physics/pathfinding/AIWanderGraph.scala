...
/** Constructs a graph for the mobs to live in, avoiding obstacles in the way.
  *
  * The algorithm to build the graph is quite naive but I believe fast enough for the purpose of
  * BFF.
  *
  *   - We "inflate" all the obstacles to get the maximum area where a mob of a given radius can
  *     live in
  *   - For each pair of these newly created obstacles, we keep only those that do not cross another
  *     obstacle. That's it!
  */
object AIWanderGraph {
  ...
  def addVertices(
      ...
      neighboursMap: Map[Complex, Vector[Complex]],
      quadTree: ShapeQT,
      inflatedEdges: Iterable[Segment],
      forceInclusion: Boolean = false
  ): Map[Complex, Vector[Complex]] =
    vertices.foldLeft(neighboursMap) { case (map, vertex) =>
      addVertex(vertex, map, quadTree, inflatedEdges, forceInclusion)
    }
  ...
  def apply(quadTree: ShapeQT, radius: Double): (FiniteGraph, Vector[Segment]) = {
    ...
    def sliceSegment(segment: Segment, otherSegments: Iterable[Segment]): Iterable[Segment] = {
      val allIntersections =
        otherSegments.map(_.intersectionPoint(segment)).collect { case Some(intersection) =>
          intersection
        }

      val newVertices =
        segment.z1 +: (allIntersections.toList.sortBy(segment.z1.distanceTo) :+ segment.z2)
    ...
    val neighboursMap = neighboursMapOneWay.toVector
      .flatMap { case (z, zs) => zs.flatMap(w => List(z -> w, w -> z)) }
      .groupBy(_._1)
      .map { case (key, value) => key -> value.map(_._2).distinct }

    (FiniteGraph(allVertices, neighboursMap, quadTree, inflatedEdges), inflatedEdges)
  }
