...
/** An [[EntityCastingInfo]] gathers everything there is to know about an entity currently casting a
  * spell. Namely, its id, the ability currently casting, the time it started casting it, the
  * position it was in when it started to cast the ability.
  *
...
final case class EntityCastingInfo(
  ...
  inline def isCasting(time: Long, delay: Long): Boolean = remainingCastingTime(time, delay) >= 0
  
  def remainingCastingTime(time: Long, delay: Long): Long = castingTime - (time + delay - startedTime)
}
