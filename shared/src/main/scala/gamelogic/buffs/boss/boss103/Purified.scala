...
/** Deals `appearanceDamage` when the buff is applied, and silence the bearer for 4 seconds. Silence
  * means that the bearer can't use an ability that require them to use mana.
  *
  * This debuff should be avoided by Pentagons and Hexagons during the game, except when the boss
  * uses its Punishment ability.
  */
final case class Purified(
  ...
  def actionTransformer(gameAction: GameAction): Vector[GameAction] = gameAction match {
    case EntityStartsCasting(_, _, _, ability)
        if ability.resource == Resource.Mana && ability.casterId == bearerId =>
      Vector.empty // prevent abilities using mana from being used
