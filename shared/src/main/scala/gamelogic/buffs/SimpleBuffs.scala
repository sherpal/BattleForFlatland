...
import gamelogic.buffs.abilities.classes.{Silenced, TriangleStunDebuff, UpgradeDirectHit}
...
/** A lot of buffs (especially the "never ending" ones) simply require
  ...
  *
  * All these buffs will be grouped into a single [[gamelogic.gamestate.GameAction]] so that we
  * drastically reduce the number of game actions needed to be created.
  */
object SimpleBuffs {

  final val simpleBuffs
      : Map[Buff.ResourceIdentifier, (Buff.Id, Entity.Id, Entity.Id, Long) => Buff] = Map(
