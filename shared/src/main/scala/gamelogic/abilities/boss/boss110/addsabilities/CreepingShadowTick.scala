...
/** Cast by the (normaly) unique instance of [[CreepingShadow]] when it collides the boss. It deals
  * damages to every player.
  *
  * The rationale behind this is that the [[CreepingShadow]] moves towards the center of mass of
  * small guies, with a radius proportional to their number. The goal for the players is then to
  * move the small guies away from the boss so that the [[CreepingShadow]] does not cast this.
  */
final case class CreepingShadowTick(useId: Ability.UseId, time: Long, casterId: Entity.Id)
  ...
  def createActions(gameState: GameState)(using IdGeneratorContainer): Vector[GameAction] =
    gameState.players.values.toVector
      .map(_.id)
      .map(EntityTakesDamage(genActionId(), time, _, metadata.damageOnTick, casterId))
  ...
  def canBeCast(gameState: GameState, time: Long): Option[String] =
    (for {
      creepingShadow <- CreepingShadow
        .extractCreepingShadow(gameState, casterId)
        .toRight("CreepingShadow is not there")
      boss <- gameState.bosses.get(creepingShadow.sourceId).toRight("Boss is not there")
      _ <- Either.cond(
        creepingShadow.collides(boss, time),
        (),
        "Creeping Shadow does not collide the boss"
      )
    } yield ()).swap.toOption
