...
/** This [[gamelogic.gamestate.serveractions.ServerAction]] triggers the actions of all the ticker
  * buffs in the game for which their last tick was further back in time than their tick rate.
  *
  * We set the time for their actions at the last tick plus the tick rate, so that the ticks are at
...
final class ManageTickerBuffs extends ServerAction {
  def apply(currentState: ActionGatherer, nowGenerator: () => Long)(implicit
    ...
    val (nextCollector, oldestTime, idsToRemove) =
      currentState.masterAddAndRemoveActions(tickActions)
