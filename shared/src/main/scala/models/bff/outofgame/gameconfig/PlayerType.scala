...
/** Determines what kind of player is set in the config. The three possibilities are Human (when the player will be
  * controlled by a human), ArtificialIntelligence (when the player will be handled by the server) or Observer (when a
  * human wants to look at the game but not play.)
  */
...
object PlayerType {
  ...
  given Decoder[PlayerType] = deriveDecoder[PlayerType]
  given Encoder[PlayerType] = deriveEncoder[PlayerType]

  given Pointed[PlayerType] = Pointed.factory(Human)
