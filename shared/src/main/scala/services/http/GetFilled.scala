...
final class GetFilled[R] private[http] () {
  ...
  def apply[T, Q](path: Path[T], query: Query[Q])(t: T, q: Q)(
      implicit decoder: Decoder[R]
  ): ZIO[HttpClient, Throwable, R] = ZIO.accessM(_.get[HttpClient.Service].get[R](path, query)(t, q))
  ...
  def apply[Q](path: Path[Unit], query: Query[Q])(q: Q)(implicit decoder: Decoder[R]): ZIO[HttpClient, Throwable, R] =
    ZIO.accessM(_.get[HttpClient.Service].get[R](path, query)(q))
