...
object AntiChamberClientTyped {
  ...
  final case class HelloFromAntiChamber(ref: ActorRef[WebSocketProtocol]) extends SetupMessage

  def apply(
      outerWorld: ActorRef[WebSocketProtocol],
      joinedGameDispatcherRef: ActorRef[Any],
      gameId: String,
      user: User
  ): Behavior[Message] =
  ...
  private def receiver(
      outerWorld: ActorRef[WebSocketProtocol],
      antiChamberRef: ActorRef[WebSocketProtocol]
  ): Behavior[Message] =
    Behaviors
