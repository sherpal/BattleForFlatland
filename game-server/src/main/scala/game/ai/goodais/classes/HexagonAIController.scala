...
trait HexagonAIController extends GoodAIController[Hexagon] {
  ...
  /**
    ...
    * @param time time for casting the ability
    * @param me instance of the [[Hexagon]]
    * @param threshold desired threshold to apply (between 0 and 1, otherwise quite useless.)
    */
  def maybeFlashHealWithThreshold(
    ...
    for {
      ...
      ability <- maybeAbilityUsage(me, FlashHeal(0L, time, me.id, target.id), gameState).startCasting
    } yield ability
  ...
  /**
    ...
    *
    * An example of filtering could be if I only have to take care on one tank, and not another one, or
    * I want to filter only on a [[Square]] which is in sight.
    *
  ...
  /**
    * Casts a [[HexagonHot]] on the first entity defined in the list of entities such that it is legal to
    * cast [[HexagonHot]] on it. Returns None if no such entity exists.
    */
  def putHotOnFirstDefined(
    ...
    entities
      .map(
        maybeEntity =>
          maybeEntity.flatMap(
            target => maybeAbilityUsage(me, HexagonHot(0L, time, me.id, target.id), gameState).startCasting
          )
      )
